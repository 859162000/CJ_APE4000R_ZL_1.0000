...
INT32 CManageBusinessFunc::GetTaxpayerInfo(CYWXML_GY &ywxml_gy, CDataUserInfo &dataUserInfo, string &strCurTime, string &strErr)
{
	INT32 ret = SUCCESS;
	ret = g_pBusBase->SKPXXCX_Business(ywxml_gy, &dataUserInfo, strCurTime, strErr);

	return g_pBusBase->ErrParse(ret, strErr);
}


INT32 CManageBusinessFunc::GetBSPInfo(CYWXML_GY &ywxml_gy, CDataUserInfo &dataUserInfo, string &strErr)
{
	INT32 ret = SUCCESS;
	ret = g_pBusBase->BSPXXCX_Business(ywxml_gy, &dataUserInfo, strErr);
	
	return g_pBusBase->ErrParse(ret, strErr);
}

INT32 CManageBusinessFunc::GetJKSJ(CYWXML_GY &ywxml_gy, CDataInvKind &dataInvKind, string &strErr)
{
	INT32 ret = SUCCESS;
...
INT32 CManageBusinessFunc::GetTaxRateInfo(CYWXML_GY &ywxml_gy, CDataTax *pDataTax, UINT8 &taxNum, string &strErr)
{
	INT32 ret = SUCCESS;
 	ret = g_pBusBase->SQSLCX_Business(ywxml_gy, pDataTax, taxNum, strErr);
	DBG_PRINT(("ret = %d", ret));
	ret = g_pBusBase->ErrParse(ret, strErr);
...
INT32 CManageBusinessFunc::NetParaManage(CYWXML_GY &ywxml_gy, CDataNetPara *pDataNetPara, string &strErr)
{
	INT32 ret = SUCCESS;

	ret = g_pBusBase->NetPara_Business(ywxml_gy, pDataNetPara, strErr);
	ret = g_pBusBase->ErrParse(ret, strErr);
