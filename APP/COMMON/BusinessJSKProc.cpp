...
// 	return ret;
	...
	CFpsc SKJFpsc(ywxml_gy, Fpmx, Fpzs, Slxlh);
...
INT32 CBusinessJSKProc::NETLXXXSC_Business(CYWXML_GY &ywxml_gy, CDataInvKind *invkind, string strQtxx, string &strLzkzxx, string &strErr)
{
	...
	DBG_PRINT(("m_serverIP= %s",p->m_serverIP.c_str()));
	DBG_PRINT(("m_serverPort= %s",p->m_serverPort.c_str()));
	DBG_PRINT(("m_serverFile= %s",p->m_serverFile.c_str()));
	g_gNetArg->SetServPara(p->m_serverIP, p->m_serverPort, p->m_serverFile);
	g_gNetArg->SetNsrsbh(ywxml_gy.m_nsrsbh);
	g_gNetArg->SetZskl(ywxml_gy.m_zskl);
